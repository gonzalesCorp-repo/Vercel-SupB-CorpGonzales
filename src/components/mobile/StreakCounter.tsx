...
export default function StreakCounter({ streak, streakMax }: StreakCounterProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-slate-900/50 backdrop-blur-md border border-slate-800 rounded-3xl p-6 flex flex-col items-center justify-center relative overflow-hidden"
    >
      {/* Background glow */}
      ...
      <div className="relative z-10 text-center">
        {isZero ? (
          <>
            <h3 className="text-slate-300 font-bold text-lg mb-1">Sin racha</h3>
            <p className="text-slate-500 text-sm">❄️ Inicia tu racha hoy</p>
          </>
        ) : (
          <>
            <div className="flex items-baseline justify-center space-x-1">
              <span className="text-5xl font-black text-slate-100">{streak}</span>
              <span className="text-slate-400 font-medium">días</span>
            </div>
            <p className="text-slate-400 text-sm mt-1">días consecutivos</p>
          </>
        )}
      </div>

      {!isZero && streakMax > 0 && (
        <div className="mt-4 pt-4 border-t border-slate-800/50 w-full text-center relative z-10">
          <p className="text-xs text-slate-500">Mejor racha: <span className="text-slate-300 font-bold">{streakMax}</span> días</p>
        </div>
      )}
    </motion.div>
  )
