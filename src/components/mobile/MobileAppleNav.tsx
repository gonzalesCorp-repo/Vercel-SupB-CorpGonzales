...
export function MobileAppleNav({ activeHub, onSelectHub }: MobileAppleNavProps) {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-slate-950/95 backdrop-blur-2xl border-t border-slate-900 px-3 pt-2 pb-2.5 safe-bottom max-w-md mx-auto select-none shadow-2xl">
      <div className="flex items-center justify-around">
        ...
        >
          <div className={`p-1.5 rounded-xl transition-colors ${
            activeHub === 'agenda' ? 'bg-indigo-500/20 text-indigo-400 shadow-sm shadow-indigo-500/20' : ''
          }`}>
            <Calendar className="w-5 h-5" />
          </div>
          <span className="text-[10px] tracking-tight">Agenda</span>
          {activeHub === 'agenda' && (
            <motion.span layoutId="navDot" className="w-1 h-1 rounded-full bg-indigo-400 absolute bottom-0" />
          )}
        </button>
        ...
        >
          <div className={`p-1.5 rounded-xl transition-colors ${
            activeHub === 'cartera' ? 'bg-indigo-500/20 text-indigo-400 shadow-sm shadow-indigo-500/20' : ''
          }`}>
        ...
        >
          <div className={`w-13 h-13 rounded-2xl flex items-center justify-center text-white shadow-xl transition-all ${
            activeHub === 'estacion'
              ? 'bg-gradient-to-tr from-indigo-600 via-purple-600 to-pink-500 shadow-purple-600/40 scale-105 ring-4 ring-slate-950'
              : 'bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 ring-4 ring-slate-950'
          }`}>
            <Layers className="w-6 h-6" />
          </div>
          <span className={`text-[10px] font-black tracking-tight mt-1 ${
            activeHub === 'estacion' ? 'text-indigo-400' : 'text-slate-500'
          }`}>
            Estación
          </span>
        </button>
        ...
        >
          <div className={`p-1.5 rounded-xl transition-colors ${
            activeHub === 'liquidacion' ? 'bg-indigo-500/20 text-indigo-400 shadow-sm shadow-indigo-500/20' : ''
          }`}>
        ...
        >
          <div className={`p-1.5 rounded-xl transition-colors ${
            activeHub === 'cuenta' ? 'bg-pink-500/20 text-pink-400 shadow-sm shadow-pink-500/20' : ''
          }`}>
            <User className="w-5 h-5" />
          </div>
          <span className="text-[10px] tracking-tight">Mi Cuenta</span>
          {activeHub === 'cuenta' && (
            <motion.span layoutId="navDot" className="w-1 h-1 rounded-full bg-pink-400 absolute bottom-0" />
          )}
        </button>
