      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-4 shadow-xl">
        <div className="relative">
          <input 
            ...
            className="w-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 placeholder-slate-500 pl-10 pr-4 py-3.5 rounded-2xl text-xs border border-slate-200 dark:border-slate-800 focus:outline-none focus:border-emerald-500 transition"
          />
      ...
      {clientesEncontrados.length > 0 ? (
        <div className="space-y-3 pt-2">
          {clientesEncontrados.map(c => (
            <div key={c.id} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex justify-between items-center shadow-lg">
              <div>
                <h4 className="font-bold text-slate-900 dark:text-slate-100 text-sm">{c.nombre}</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">DNI: {c.dni || 'N/A'} • Cel: {c.celular || 'N/A'}</p>
              </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 text-center space-y-3 shadow-xl">
          ...
          <h3 className="font-bold text-slate-900 dark:text-slate-200 text-base">Sin asignaciones</h3>
          ...
          >
            ...
            >
              <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-3">
                ...
                <button onClick={() => setShowAddClienteModal(false)} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white">
                  <X className="w-5 h-5" />
                </button>
              </div>
