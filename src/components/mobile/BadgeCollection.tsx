...
export default function BadgeCollection({ earnedBadges, allBadges }: BadgeCollectionProps) {
  ...
  return (
    <div className="bg-slate-900/50 backdrop-blur-md border border-slate-800 rounded-3xl p-5">
      <h3 className="text-slate-100 font-black text-lg mb-4">Insignias</h3>
        ...
        {allBadges.map((badge, index) => {
          ...
          return (
            ...
            >
              <div 
                className={`w-16 h-16 rounded-full flex items-center justify-center text-3xl mb-2 transition-all duration-300 relative
                  ${isEarned 
                    ...
                    : 'bg-slate-800 border-2 border-slate-700 grayscale opacity-60'
                  }
                `}
              >
                {badge.icono}
              ...
              <span className={`text-[10px] font-bold text-center leading-tight line-clamp-2 ${isEarned ? 'text-slate-300' : 'text-slate-600'}`}>
                {badge.nombre}
              </span>
              ...
              <AnimatePresence>
                {selectedBadge?.id === badge.id && isEarned && (
                  <motion.div 
                    ...
                    className="absolute z-20 top-full mt-2 w-48 bg-slate-800 border border-slate-700 rounded-xl p-3 shadow-xl left-1/2 -translate-x-1/2"
                  >
                    ...
                    <div className="relative z-10 text-center">
                      <p className="text-slate-100 font-bold text-sm mb-1">{badge.nombre}</p>
