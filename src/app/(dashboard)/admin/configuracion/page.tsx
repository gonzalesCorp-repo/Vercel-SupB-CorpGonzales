...
export default function WFMConfigPage() {
  ...
  const openNewModal = () => {
    setEditId(null);
    setFormData({ nombre: '', estado_destino: 'DISPONIBLE', actualiza_timestamp: false, penaliza_cola: false, color: 'bg-slate-100 text-slate-700' });
    setIsModalOpen(true);
  };

  const openEditModal = (conf: ConfigPeticion) => {
    setEditId(conf.id);
    setFormData({ nombre: conf.nombre, estado_destino: conf.estado_destino, actualiza_timestamp: conf.actualiza_timestamp, penaliza_cola: conf.penaliza_cola, color: conf.color });
    setIsModalOpen(true);
  };
    ...
    <div className="p-8 h-full bg-slate-50/50 min-h-[calc(100vh-4rem)]">
            ...
            <table className="w-full text-sm text-left text-slate-600">
              <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-6 py-4">Reinicio de Turno</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {configs.map(conf => (
                  <tr key={conf.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-xs font-bold text-slate-500 uppercase">{conf.estado_destino}</span>
                    </td>
                    <td className="px-6 py-4">
                      {conf.actualiza_timestamp ? (
                        <span className="text-orange-600 font-bold bg-orange-50 px-2 py-1 rounded-md text-xs">Sí (Va al final de la cola)</span>
                      ) : (
                        <span className="text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded-md text-xs">No (Conserva antigüedad)</span>
                      )}
                    </td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editId ? "Editar Petición" : "Nueva Petición"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Estado de Sistema (Destino)</label>
            <select 
              className="w-full px-4 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500"
              value={formData.estado_destino}
              onChange={e => setFormData({...formData, estado_destino: e.target.value})}
            >
              <option value="DISPONIBLE">DISPONIBLE (Listo para atender)</option>
              <option value="OCUPADO">OCUPADO (Sigue en cola, pero pausado)</option>
              <option value="INACTIVO">INACTIVO (Sale del monitor visualmente)</option>
            </select>
          </div>
          ...
          <div className="pt-2">
            <label className="flex items-center gap-3 cursor-pointer p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
              <input 
                ...
                checked={formData.actualiza_timestamp}
                onChange={e => setFormData({...formData, actualiza_timestamp: e.target.checked})}
              />
              <div>
                <div className="font-bold text-slate-800 text-sm">Actualiza Timestamp (Reinicia Turno)</div>
                <div className="text-xs text-slate-500 mt-0.5">Al regresar, el operario perderá su antigüedad y se irá al fondo de la cola de disponibles.</div>
              </div>
            </label>
          </div>
