...
type SoporteTab = 'recepcion' | 'caja' | 'despacho';
...
export default function MobileSoportePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 pb-24 font-sans selection:bg-indigo-500/30">
      
      {/* Top Header */}
      <div className="flex items-center justify-between bg-slate-900/90 border border-slate-800 p-3.5 rounded-2xl mb-4 backdrop-blur-md shadow-lg">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-600 to-indigo-500 flex items-center justify-center font-black text-white text-sm shadow-md shadow-indigo-950">
            S
          </div>
          <div>
            <div className="flex items-center gap-1.5">
              <h1 className="text-xs font-black text-white">{agente.nombre}</h1>
              <span className="text-[9px] font-bold px-1.5 py-0.2 bg-indigo-500/20 text-indigo-300 rounded border border-indigo-500/30">
                SOPORTE
              </span>
            </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 text-rose-400 hover:text-rose-300 bg-rose-950/30 border border-rose-900/40 rounded-xl transition"
            title="Cerrar Sesión"
          >
      ...
      <div className="flex gap-1.5 bg-slate-900 p-1.5 rounded-2xl border border-slate-800 mb-4 shadow-md">
        {herramientas.includes('recepcion') && (
          <button
            ...
            className={`flex-1 py-2.5 px-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition ${
              activeTab === 'recepcion'
                ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-md shadow-indigo-950'
